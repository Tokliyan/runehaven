...
## Confirmed, locked spec for the next build (v32 — Abyssal Hollow, reusing v29's cave-interior system)

v30 shipped successfully. Reuses v29's proven system exactly — generalized
to a second biome, not redesigned. Confirmed: `uwcaveClusterAnchor()` and
`enterInterior()` are currently hardcoded to `B.UWCAVE`; `shadow_dragon`
currently spawns on the surface (`biomes: [B.ABYSSAL]`), same as
`water_dragon` did before v29 moved it inside.

**Generalize, don't duplicate:**
- `uwcaveClusterAnchor(tx,ty)` → `clusterAnchor(tx,ty,biomeConst)`, same
  flood-fill, parameterized.
- `enterInterior(tx,ty)` → add a `biomeConst` param, space id becomes
  `"cave:" + kind + ":" + anchor` where `kind` is `"uwcave"` or `"abyssal"`.
- Entry trigger: touching a `B.ABYSSAL` tile while diving, same as UWCAVE.
- `buildInterior()`: add a palette branch — Abyssal reads colder/darker
  (near-black blue, sparse glow) vs the cave-tone blue-grey UWCAVE uses.
  Same 26x26 grid, same generation, different `col` values only.

**Species — moved in, not duplicated:**
- `shadow_dragon` → `biomes: []`, spawns inside the generated Abyssal
  interior instead (same pattern as Water Dragon/Sea Serpent in v29).

**Resource — genuinely undecided until now, deciding here:**
- No bible text names one for Abyssal Hollow the way "rare aquatic
  resources" was named for Underwater Caves. Propose `void_shard` (color:
  deep violet), same gatherable pattern as `aquatic_essence`, no recipe
  yet — matches the "exists and is gatherable" scope v29 set.

**Proof gates:** same shape as v29's — determinism, shared-space filter
(already generic, no change needed there), Shadow Dragon spawns inside
only, `void_shard` gatherable, exit restores surface position.

**After v32 ships, wait for `NEXT_BUILD.md` to be updated — do not
auto-start another version.**
